...
Support multi-thèmes. Adapté depuis sfd/methods/html_renderer.py.
"""
...
<div class="cover">
  ...
  <div class="cover-band">Document généré automatiquement</div>
</div>
<hr class="page-sep">
{hist_html}
{docs_html}
...
(function() {{
  ...
  function showToolbar(msg) {{
    ...
    saveTimer = setTimeout(function() {{ toolbar.classList.remove('visible'); }}, 2500);
  }}
  document.addEventListener('dblclick', function(e) {{
    ...
    e.preventDefault();
    e.stopPropagation();
    if (activeEl && activeEl !== el) {{ activeEl.contentEditable = 'false'; }}
  ...
  document.addEventListener('focusout', function(e) {{
    ...
    if (!path) return;
    window.parent.postMessage({{ type: 'sfd-edit', path: path, value: value }}, '*');
  ...
  document.addEventListener('keydown', function(e) {{
    var el = e.target;
    if (!el.getAttribute('data-editable') || el.contentEditable !== 'true') return;
    if (e.key === 'Enter' && (el.tagName === 'P' || el.tagName === 'LI')) {{ e.preventDefault(); el.blur(); }}
    if (e.key === 'Escape') {{ el.contentEditable = 'false'; activeEl = null; toolbar.classList.remove('visible'); }}
  }});
}})();
(function() {{
  ...
  waitMermaid(function() {{
    ...
    function next() {{
      ...
      try {{ code = JSON.parse(sc.textContent); targetId = sc.getAttribute('data-id'); el = document.getElementById('mermaid-' + targetId); }} catch(e) {{ next(); return; }}
      if (!el) {{ next(); return; }}
      var uid = 'msv' + (counter++) + String(Date.now());
      function showErr(err) {{
        var msg = err && err.message ? err.message : String(err);
        el.innerHTML = '<div style="background:#fff3cd;border:1px solid #ffc107;border-radius:4px;padding:10px 12px;margin-bottom:6px;">'
          + '<strong style="color:#856404;">⚠ Erreur Mermaid :</strong> '
          + '<span style="color:#856404;font-size:9pt;">' + msg.replace(/</g,'&lt;').substring(0,300) + '</span></div>'
          + '<pre class="mermaid-fallback">' + code.replace(/</g,'&lt;') + '</pre>';
        next();
      }}
      mermaid.render(uid, code).then(function(r) {{ el.innerHTML = r.svg; next(); }}).catch(function(err) {{
        var msg = err && err.message ? err.message : String(err);
        ...
        if ((isFlowErr || isSvgErr) && /^(flowchart|graph)/i.test(code.trim())) {{
          ...
          mermaid.render(uid + 'r', code2).then(function(r2) {{ el.innerHTML = r2.svg; next(); }}).catch(function(e2) {{ showErr(e2); }});
        }} else if (isSvgErr) {{
          setTimeout(function() {{
            mermaid.render(uid + 't', code).then(function(r3) {{ el.innerHTML = r3.svg; next(); }}).catch(function(e3) {{ showErr(e3); }});
          }}, 200);
        }} else {{ showErr(err); }}
      }});
    }}
