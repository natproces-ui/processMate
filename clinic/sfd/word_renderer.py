"""
word_renderer.py — Génération du document Word SFD avec support multi-thèmes.
Adapté depuis sfd/methods/word_renderer.py.
"""
...
from .html_renderer import fix_mermaid
...
def _write_cover(doc, sfd: SFDDocument, theme: SFDTheme):
    ...
    for edge in ("top", "bottom"):
        ...
        pBdr.append(b)
    pPr.append(pBdr)
    _run(p_band, "  Document généré automatiquement",
...
def _write_acteurs(doc, sfd: SFDDocument, theme: SFDTheme):
    ...
    if not sfd.acteurs:
        ...
        return
    rows = [[a.id, a.nom, a.type, a.role, a.description] for a in sfd.acteurs]
...
def _mermaid_to_png(mermaid_code: str) -> bytes | None:
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "ProcessMate-SFD/1.0"})
