...
export default function InitForm({ onSubmit, isLoading, progressMessage }: InitFormProps) {
  ...
  return (
    <div className="flex flex-col h-full">
              ...
              <ul className="mt-2 space-y-1">
                {files.map((f, i) => (
                  <li key={i} className="flex items-center gap-2 px-2 py-1.5 bg-slate-50 rounded text-xs">
                    ...
                    {!isLoading && (
                      <button type="button" title="Supprimer ce fichier" onClick={() => removeFile(i)}>
              ...
              {urls.map((url, i) => (
                <div key={i} className="flex gap-2">
                  ...
                  {urls.length > 1 && !isLoading && (
                    <button type="button" title="Supprimer cette URL" onClick={() => removeUrl(i)} className="p-2 text-slate-400 hover:text-red-500">
                      <X className="w-4 h-4" />
                    </button>
                  )}
                </div>
      ...
      {/* Bouton submit — toujours visible en bas */}
      <div className="flex-shrink-0 p-4 pt-3 border-t border-slate-100 bg-white">
        <button
          type="button"
