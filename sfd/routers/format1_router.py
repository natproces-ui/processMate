...
from methods.format1_methods import process_format1_extraction
import json
import os
...
@router.post("/extract-json", response_model=Format1SFD)
async def extract_json_format1(
    file: Optional[UploadFile] = File(None),
    text: Optional[str] = Form(None)
):
    """
    Extraire et générer un SFD au Format 1 - Sortie JSON en réponse (copier directement)
    ...
    Returns:
        Format1SFD: SFD structuré au format JSON
    ...
    try:
        ...
        sfd = await process_format1_extraction(file=file, text=text)
        return sfd
...
async def extract_download_format1(
    file: Optional[UploadFile] = File(None),
    text: Optional[str] = Form(None)
):
    """
    Extraire et générer un SFD au Format 1 - Sortie fichier JSON téléchargeable
    ...
    try:
        ...
        filename = f"{sfd.nom_projet}_format1_{timestamp}.json"
...
async def download_json_format1(filename: str):
    """
    Télécharger un fichier JSON SFD Format 1 sauvegardé
...
async def health_check():
    """Vérifier que le service Format 1 fonctionne"""
    return {"status": "healthy", "format": "format1"}